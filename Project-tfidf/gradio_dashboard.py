...
with open("tfidf_artifacts.pkl", "rb") as f:
    tfidf_artifacts = pickle.load(f)
    tfidf_vectorizer = tfidf_artifacts["vectorizer"]
    tfidf_matrix = tfidf_artifacts["matrix"]


# --- Recommendation Logic (TF-IDF only) ---
...
with gr.Blocks(theme=gr.themes.Soft(), css=css) as dashboard:
    gr.Markdown("# TF-IDF Book Recommendation System")
